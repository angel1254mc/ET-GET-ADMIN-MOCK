...
function Pagination({curr_page, total_pages, active_href = "glossary", searchQuery= "", DynamicRouteTest}) {
    ...
    const routeInputPage = () => {
        router.push({
            ...
            query: {page: desiredPage, search: searchQuery}
        },
        undefined
        );

    }
    const [desiredPage, setDesiredPage] = useState(curr_page); //Serves to keep track of the page the user wishes to jump to
    ...
    <div className={PagiStyles.pagi_wrapper}>
        <div className={PagiStyles.chevron}><ChevronLeft className={PagiStyles.chevron_svg}/></div>
        <div className={PagiStyles.pagi_inner}>
            <input className={PagiStyles.page_input}
                ...
                onChange={(event) => {
                    setDesiredPage(event.target.value);
                    }
                }
                onKeyDown={e => {
                        if (e.key === 'Enter')
        ...
        <div className={PagiStyles.chevron}><ChevronRight className={PagiStyles.chevron_svg}/></div>
    </div>
  )
}
